...
\subsection{Beispiel: nicht halbeinfache kommutative, unitale Banachalgebra (Ü.A. 16)}

Sei $x\neq 0$ eine nilpotente $n\times n$-Matrix und $\A$ die erzeugte unitale, kommutative Banachalgebra. Für $\varphi\in \hat \A$ gilt $\varphi(x)=0$, denn $\varphi(x)^n=\varphi(x^n)=\varphi(0)=0$, also gilt für $y=\sum_{i=0}^{n-1} a_i x^i$ die Identität $\varphi(y)=a_0$, somit $\hat \A$ Einpunktmenge und das Radikal ist gegeben durch $\{ y\in \A: a_0=0\}$.
...
Auf $\mathrm{M}_n$ definiert $\tau(x)=\frac{1}{n} \Spur(x)$ den \emph{Spurzustand}. Es ist $\Hc=\mathrm{M}_n$ und $\E_{ij}=\sqrt{n} e_{ij}$ ist ONB\@. Schreibe $\Hc=\oplus_{i=1}^n \C^n$, dann ist die Linksreguläre Darstellung $\pi(x)$ durch $\left(\begin{smallmatrix} x & & \\ & \ddots & \\ & & x \end{smallmatrix}\right)$ gegeben.
